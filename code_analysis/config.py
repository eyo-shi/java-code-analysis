...
# Defaults aligned with .project-metadata.yaml.
...
def diagnose_environment() -> None:
    ...
    print(
        "CML injects AMP Configuration into os.environ at task startup. "
        "Deploy values are also editable in Project Settings > Advanced."
    )
    for name in MANAGED_ENV_VARS:
        ...
        if raw is None or not str(raw).strip():
            ...
            if default is not None:
                print(f"  {name}: MISSING (metadata default: {default})")
            else:
...
class Config:
    ...
    def from_env(cls) -> Config:
        ...
        if not neo4j_uri:
            ...
            raise ValueError(
                "NEO4J_URI is required. Set it in AMP Configuration at deploy time, or in "
                "Project Settings > Advanced > Environment Variables, then re-run the AMP task."
            )
